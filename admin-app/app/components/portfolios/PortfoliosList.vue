<template>
  <div class="space-y-4">
    ...
    <div v-else class="bg-white rounded-lg shadow p-8 text-center text-gray-500">
      <p>Please select an organization and ledger to view portfolios</p>
    </div>
...
import PortfoliosTable from "./PortfoliosTable.vue";
...
const pagination = computed(() => ({
  ...
  total: portfoliosStore.total || 0,
}));

const filteredItems = computed(() => {
  return portfoliosStore.items.filter((item: Portfolio) => {
    const matchesSearch =
...
const handleNextPage = async () => {
  if (currentPage.value * Number(pageLimit.value) < (portfoliosStore.total || 0)) {
    currentPage.value++;
...
const handleDelete = async () => {
  ...
  try {
    await portfoliosStore.delete(deleteTargetId.value);
