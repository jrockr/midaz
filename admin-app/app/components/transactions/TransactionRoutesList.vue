<template>
  <div class="space-y-4">
    ...
    <div class="flex justify-between items-center">
      <h2 class="text-lg font-semibold">Transaction Routes</h2>
      <button
        @click="$emit('create')"
    ...
    >
      <div class="bg-white rounded-lg p-6 max-w-sm w-full mx-4">
        ...
        <p class="text-gray-600 mb-6">
          Are you sure you want to delete this transaction route?
        </p>
...
import TransactionRoutesTable from "./TransactionRoutesTable.vue";
...
const pagination = computed(() => ({
  ...
  total: transactionsStore.total || 0,
}));

const filteredItems = computed(() => {
  return transactionsStore.items.filter((item: TransactionRoute) => {
...
const handleNextPage = async () => {
  if (currentPage.value * Number(pageLimit.value) < (transactionsStore.total || 0)) {
    currentPage.value++;
    await loadRoutes();
  }
};
...
const handleDelete = async () => {
  ...
  try {
    await transactionsStore.delete(deleteTargetId.value);
...
const loadRoutes = async () => {
  try {
    ...
    showToast("Failed to load transaction routes", "error");
  }
};
